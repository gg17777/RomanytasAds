...
Future<bool> removeUserFromExistingEventOnSameDay(
  ...
  try {
    ...
    if (newEventDate == null) return false; // If no date, no need to remove
    ...
    for (var participationDoc in userEventsSnapshot.docs) {
      ...
      // If the event has the same date, remove the user
      if (formattedExistingEventDate == formattedNewEventDate) {
        await participationDoc.reference.delete();
        print("User removed from event: ${eventDoc.id}");
        return true; // User was removed
      }
    }

    return false; // No conflicting event found, nothing removed
  } catch (error) {
    print("Error removing user from existing event: $error");
    return false;
  }
}
