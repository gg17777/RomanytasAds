...
Future<List<SwipeStruct>> getExclusiveSwipes(
  String eventID,
  ...
  try {
    ...
    final swipeActionsQuery = await firestore
        .collection(secretParties)
        .doc(eventID)
        .collection('secret_party_swipeActions')
    ...
    final participantsQuery = await firestore
        .collection(secretParties)
        .doc(eventID)
        .collection('secret_party_participating')
        .get();
    ...
    List<SwipeStruct> participants = participantsQuery.docs.map((doc) {
      ...
      return SwipeStruct(
          ...
          likesNotifications: data["matches_notifications"] as bool?);
    }).toList();
